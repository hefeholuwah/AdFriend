document.addEventListener("DOMContentLoaded", () => {
  ...
  const pointsDisplaySpan = document.getElementById("pointsDisplay"); // New element for points
  ...
  // Load user points from chrome.storage.local
  function updatePointsDisplay() {
    chrome.storage.local.get({ userPoints: 0 }, (result) => {
      pointsDisplaySpan.innerText = result.userPoints;
    });
  }
  updatePointsDisplay();
  
  // Listen for messages to update points display in real time
